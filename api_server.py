...
def register():
    ...
    try:
        ...
        conn.commit()
        return jsonify({"status": "sucesso", "mensagem": "Utilizador registado com sucesso!"}), 201
...
def login():
    ...
    try:
        ...
        if user and bcrypt.checkpw(senha.encode('utf-8'), user['senha_hash'].encode('utf-8')):
            aluno_data = { "id": str(user['id']), "nome": user['nome'], "pontuacao_total": user['pontuacao_total'] }
